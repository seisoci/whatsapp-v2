...
function MenuTriggerButton({ name }: { name: LithiumMenuItemsKeys }) {
  ...
  if (isDirectLink && directLinkHref) {
    return (
      <Link
        href={directLinkHref}
        className={cn(
          'inline-flex items-center gap-1 text-sm font-medium leading-5 transition-all duration-200',
          isDirectLinkActive
            ? 'text-primary'
    ...
    <>
      <span
        className={cn(
          ...
          isActive(menuItem.dropdownItems)
            ? 'text-primary'
      ...
      <span
        className={cn(
          ...
          isActive(menuItem.dropdownItems!) && 'text-primary'
        )}
      >
        <PiCaretDownBold />
      </span>
