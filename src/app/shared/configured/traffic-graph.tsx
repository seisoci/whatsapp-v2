...
import { getOnuTrafficGraph } from '@/lib/sanctum-api';
import { Chart, registerables } from 'chart.js';
import 'chartjs-adapter-date-fns';
import { PiArrowCounterClockwiseBold } from 'react-icons/pi';
import { formatTraffic } from '@/utils/format-traffic';
...
export default function TrafficGraph({ onuId, onuName }: TrafficGraphProps) {
  ...
  const formatBps = (bitsPerSec: number): string => {
    return formatTraffic(bitsPerSec);
  };
  ...
  const generateLegend = (chart: Chart) => {
    ...
    // Reverse order: Upload first, then Download
    for (let i = datasets.length - 1; i >= 0; i--) {
      const dataset = datasets[i];
      const data = dataset.data as any[];

      // Calculate Current (last non-null value) and Maximum
      ...
      const currentStr = current !== null ? formatBps(current) : 'N/A';
      const maximumStr = maximum !== null ? formatBps(maximum) : 'N/A';
      ...
      html += `<span style="margin-left: 10px; opacity: 0.7;">Maximum: ${maximumStr}</span>`;
      html += '</li>';
    }
  ...
  const fetchSeries = async (from: number, to: number) => {
    try {
      ...
      const response = await getOnuTrafficGraph(
        onuId,
      ...
      if (response.code === 200) {
        ...
        if (response.series && response.series.length > 0) {
          datasets = response.series.map((s: any, index: number) => {
            const colors = [
              { border: '#4285F4', bg: '#4285F4' },  // blue for Download
              { border: '#F58411', bg: '#F58411' }   // orange for Upload
            ];
          ...
          // Create empty datasets to show time axis even with no data
          datasets = [
            {
              label: 'Download',
              data: [],
            ...
            tooltip: {
              ...
              callbacks: {
                label: function(context) {
                  ...
                  const bitsPerSec = context.parsed.y;
                  label += formatTraffic(bitsPerSec);
                  return label;
                }
              }
            }
  ...
  return (
    <div className="graph-item">
      <div id="onu_traffic_container">
        <div id="onu_traffic_wrap" className="rounded-lg border border-gray-200 bg-white p-4">
          ...
          <div id="onu_traffic_chart_container" className="bg-gray-100 p-2" style={{ height: '170px', cursor: 'grab', display: hasData ? 'block' : 'none' }}>
            <canvas ref={chartRef} />
          </div>

          {hasData && (
            <div id="onu_traffic_legend" className="mt-3">
