...
import { FaInfoCircle, FaMicrochip, FaNetworkWired, FaArrowUp, FaSitemap } from 'react-icons/fa';
...
interface OltNavigationProps {
  oltId: string | number;
}

export default function OltNavigation({ oltId }: OltNavigationProps) {
  ...
  const menuItems = [
    {
      label: 'OLT Details',
      value: routes.olt.details(oltId),
      icon: FaInfoCircle,
    },
    {
      label: 'OLT Cards',
      value: routes.olt.cards(oltId),
      icon: FaMicrochip,
    },
    {
      label: 'PON Ports',
      value: routes.olt.ponPorts(oltId),
      icon: FaNetworkWired,
    },
    {
      label: 'Uplinks',
      value: routes.olt.uplinks(oltId),
      icon: FaArrowUp,
    },
    {
      label: 'VLANs',
      value: routes.olt.vlans(oltId),
      icon: FaSitemap,
    },
  ];
            ...
            {menuItems.map((menu, index) => {
              ...
              return (
                ...
                >
                  <Text
                    as="span"
                    className="inline-flex items-center gap-2 rounded-md px-2.5 py-1.5 transition-all duration-200 group-hover:bg-gray-100/70"
                  >
