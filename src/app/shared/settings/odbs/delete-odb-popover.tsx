...
import { deleteOdb } from '@/lib/sanctum-api';

export default function DeleteOdbPopover({
  odbId,
  odbName,
  onSuccess,
}: {
  ...
  const handleDelete = async (setOpen: (open: boolean) => void) => {
    setIsDeleting(true);
    try {
      await deleteOdb(odbId);
      toast.success('ODB deleted successfully');
      setOpen(false);
      if (onSuccess) onSuccess();
    } catch (error: any) {
      toast.error(error.message || 'Failed to delete ODB');
    } finally {
      setIsDeleting(false);
    }
  };
      ...
      <Popover.Content className="z-10">
        {({ setOpen }) => (
          <div className="w-56 pb-2 pt-1 text-left">
            ...
            >
              <PiTrashBold className="me-1 h-[17px] w-[17px]" /> Delete ODB
            </Title>
            <Text className="mb-2 leading-relaxed text-gray-500">
              Are you sure you want to delete <strong>{odbName}</strong>?
            </Text>
