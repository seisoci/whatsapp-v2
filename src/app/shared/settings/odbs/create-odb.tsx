...
import { z } from 'zod';
import { createOdb, getZones } from '@/lib/sanctum-api';

const createOdbSchema = z.object({
...
type CreateOdbInput = z.infer<typeof createOdbSchema>;
...
export default function CreateOdb({ onSuccess }: { onSuccess?: () => void }) {
  ...
  const onSubmit: SubmitHandler<CreateOdbInput> = async (data) => {
    setLoading(true);
    try {
      await createOdb(data);
      toast.success('ODB created successfully');
      setReset({});
      closeModal();
      if (onSuccess) onSuccess();
    } catch (error: any) {
      const errorMessage = error.message || 'Failed to create ODB';
  ...
  return (
    <Form<CreateOdbInput>
      resetValues={reset}
      onSubmit={onSubmit}
      validationSchema={createOdbSchema}
      useFormProps={{
        defaultValues: {
          name: '',
          nr_of_ports: null,
